...
SRC = $(call rwildcard, *.cpp, *.h) #CHANGED THIS (c->cpp)
#OBJS = $(SRC:$(SRC_DIR)/%.cpp=$(OBJ_DIR)/%.o) #CHANGED THIS (c->cpp)
OBJS ?= RaylibPong.cpp
...
#%.o: %.cpp #CHANGED THIS (c->cpp)
$(OBJ_DIR)/%.o: $(SRC_DIR)/%.cpp #CHANGED THIS (c->cpp)
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
	...
	rm -f *.out *.exe # CHANGED THIS
    endif
    ifeq ($(PLATFORM_OS),OSX)
		find .e -type f -perm +ugo+x -delete
		rm -f *.o
    endif
endif
ifeq ($(PLATFORM),PLATFORM_RPI)
