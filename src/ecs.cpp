...
struct ComponentContainer {
  ...
  template <typename T> Vector<T> &getComponentVector() {
...
private:
  ...
  template <typename T> void lazilyRegisterComponentOperations() {
    if (auto f = componentOperations.find(typeid(T));
        f == componentOperations.end()) {
      auto dummyPusher = [this]() {
        getComponentVector<T>().push_back(std::nullopt);
      };

      auto moveComponent = [this](ComponentContainer &oth, Entity sourceId,
                                  Entity destId) {
      ...
      auto removeComponent = [this](Entity eid) {
        getComponentVector<T>()[eid] = std::nullopt;
      };

      componentOperations.insert(
          {typeid(T),
           ComponentOperation{dummyPusher, moveComponent, removeComponent}});
    }
  }
};
...
struct Commands {
  ...
  template <typename... Ts> void addEntity(Ts &&...comps) {
...
class ECS {
  template <typename... Ts> struct MultiIterator {
    ...
    template <typename F> void forEachMut(F &&fn, Commands &cmd) {
      ...
      for (auto current = begins; current != ends;
        ...
        if (all_some) {
          auto paramTuple = std::tuple_cat(std::make_tuple(std::ref(cmd)),
                                           unwrapIterators(current));
          std::apply(fn, paramTuple);
        }
      }
    }

    template <typename F> void forEachMutId(F &&fn, Commands &cmd) {

      auto multiIter = begins;
      auto eid = 0;
      for (; multiIter != ends;
           advance_all<sizeof...(Ts) - 1>(multiIter), eid++) {

        const auto all_some = std::apply(
            [](auto &&...args) { return (args->has_value() && ...); },
            multiIter);

        if (all_some) {
          auto paramTuple = std::tuple_cat(std::make_tuple(eid),
                                           std::make_tuple(std::ref(cmd)),
                                           unwrapIterators(multiIter));
          std::apply(fn, paramTuple);
        }
      }
    }
...
public:
  ...
  void copyComponents(Commands &cmd, Entity sourceId, Entity destId) {
    for (auto &[typeId, anyVec] : cmd.entitiesToAdd.component_vectors) {
      cmd.entitiesToAdd.componentOperations[typeId].moveComponent(
          components, sourceId, destId);
    }
  }
  ...
  void processCommands(Commands &cmd) {
    ...
    for (uint32_t sourceId = 0; sourceId < cmd.entitiesToAdd.numEntities;
         sourceId++) {
  ...
  template <typename... Ts, typename F> void forEachMut(F &&fn) {
    Commands cmd{};
  ...
  template <typename... Ts, typename F> void forEachMutId(F &&fn) {
    Commands cmd{};
...
int main() {
  ...
  ecs.forEachMutId<Foo, Baz>(
      [](ecs::Entity selfId, ecs::Commands &cmd, auto f, auto b) {
        if (b->z == 100) {
